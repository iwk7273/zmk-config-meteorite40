...
- Prospector 対応作業は `feat/prospector-support` branch で行い、`main` へ直接コミットしません。
...
- 現行 manifest は `iwk7273/zmk#feat/meteorite-custom-config-rpc`、`iwk7273/zmk-feature-meteorite-config#feat/prospector-support`、`iwk7273/prospector-zmk-module#feat/meteorite-prospector-support` を参照します。
...
- `config/west.yml` は Meteorite 対応 ZMK fork、`zmk-feature-meteorite-config` module、`prospector-zmk-module` module を参照します。
- Prospector Scanner 対応 firmware では `CONFIG_ZMK_STATUS_ADVERTISEMENT=y` と `CONFIG_ZMK_STATUS_ADV_KEYBOARD_NAME` を `.conf` に設定します。
- Prospector Meteorite scanner と組み合わせる場合、追加で `CONFIG_PROSPECTOR_STATUS_ADV_V2_EXT=y` を設定し、v2 status ADV を有効化します。現行 v2 は legacy ADV の multi-frame transport を使うため、keyboard 側では `CONFIG_BT_EXT_ADV` を有効化しません。v2 ADV はレイヤー名リストと Meteorite custom config (OS / CPI / scroll layer) を 1Hz で配信し、v1 legacy ADV と並走します。
